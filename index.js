...
} = require("./interactions/handleSendMsgDisabled");
...
canvax.registerFont("./storage/DejaVuSansCondensed-Bold.ttf", {
  family: "Discordx",
});
...
////////////////////////////////////////////////////////////////////////send erorr bot in server discord
const errorChannelId = "1329264270235668591"; // استبدل هذا بالـ ID الخاص بالروم اللي تريد ترسل فيه الأخطاء

client.once("ready", () => {
  console.log(`4S System`);
});
...
// التعامل مع الأخطاء في الـ Promise الغير معالجة
process.on("unhandledRejection", (reason, promise) => {
  console.error("Promise غير معالج:", promise, "السبب:", reason);

  const channel = client.channels.cache.get(errorChannelId);
  if (channel) {
    channel
      .send(
        `⚠️ **Promise غير معالج حدث:**\n\`\`\`${reason.stack || reason}\`\`\``
      )
      .catch(console.error);
  }
});
...
// قائمة الكاتيجوريز التي يُسمح فيها بإرسال الروابط
const CATEGORY_IDS = [
  "1320530102391410690",
  "1320530153532690545",
  "1320530235631992852",
...
client.on("messageCreate", (message) => {
  // لو الرسالة من بوت أو مش في سيرفر، نتجاهلها
  if (message.author.bot || !message.guild) return;

  // التعبيرات المنتظمة للتحقق من الروابط
  const urlRegex = /https?:\/\/[^\s]+|www\.[^\s]+/i;
  const discordInviteRegex =
    /discord\.gg\/[a-zA-Z0-9]+|discord\.com\/invite\/[a-zA-Z0-9]+/i;

  // التحقق لو الرسالة تحتوي على رابط (عام أو ديسكورد)
  if (
    urlRegex.test(message.content) ||
    discordInviteRegex.test(message.content)
  ) {
    // التحقق إذا الشخص لديه أي واحدة من الرولز المسموح بها
    const member = message.guild.members.cache.get(message.author.id);
    if (ALLOWED_ROLES.some((roleId) => member.roles.cache.has(roleId))) {
      // الشخص لديه واحدة من الرولز المسموح لها، لا تفعل شيئًا
      return;
    }

    // التحقق إذا الروم موجودة تحت واحدة من الكاتيجوريات المسموح بها
    if (CATEGORY_IDS.includes(message.channel.parentId)) {
      // الروم ضمن الكاتيجوريات المسموح بها، لا تفعل شيئًا
      return;
    }
    ...
    // إنشاء Embed ككائن عادي
    const embed = {
      title: "🚫 ممنوع إرسال الروابط!",
      description:
        "تم حذف رسالتك لأنها تحتوي على رابط.\n" +
        "🚨 **المشرفين والإداريين فقط** هم المسموح لهم بإرسال الروابط.\n" +
        "يرجى الالتزام بالقوانين.",
      color: 0xff0000, // اللون بالأحمر
      footer: {
        text: "يرجى الالتزام بالقوانين",
      },
    };

    // إرسال الرد مع منشن للشخص
    message.channel
      .send({
        content: `${message.author}, ممنوع إرسال الروابط هنا!`,
        embeds: [embed],
      })
      .then((msg) => {
        // حذف رسالة الـ Embed بعد 10 ثوانٍ
        setTimeout(() => {
...
const guildID = '740299333697536061';
...
/////////////////////////////////////////////////////////////////////////////////////////////////// ROLES STATUS
const config = {
  prefix: "4!", // البريفكس الخاص بالأوامر
  roles: [
    "1321943174028984493",
    "1321942183787364362",
    "1320705763693297756",
    ...
    "1325784095124492404",
    "1319445752166088835",
  ], // استبدل بـ IDات الرولات
  channelId: "1324841382573244477", // استبدل بـ ID الروم اللي هتحدث فيه الرسالة
  messageId: "1329144173802160210", // استبدل بـ ID الرسالة لو كانت موجودة
  roleEmojis: {
    // تحديد الإيموجي لكل رول
    "1321943174028984493": "", // رول ID مع الإيموجي
    "1321942183787364362": "",
    "1320705763693297756": "",
    "1320864380920922142": "",
    "1325784095124492404": "",
  ...
  developerIds: {
    // معرفات المطورين
    developerbot: "274947190814670848", // استبدل بـ ID حسابك
    mainDeveloper: "803873969168973855", // استبدل بـ ID حسابك الآخر
  },
};

let embedMessageId = config.messageId; // هنا هنخزن الـ embed message ID
let isIntervalSet = false; // متغير لتتبع ما إذا كان الـ interval قد تم إنشاؤه بالفعل

client.once("ready", async () => {
  console.log(`${client.user.tag} is now online!`);

  // عرض رسالة لو المطور متصل
  checkDevelopersOnline();

  // إذا كان هناك معرف رسالة محفوظ مسبقًا
  if (embedMessageId) {
    try {
      // تحميل الرسالة القديمة
      ...
      // بدء التحديث التلقائي إذا لم يكن قد بدأ بالفعل
      if (!isIntervalSet) {
        setInterval(updateEmbed, 60000); // التحديث كل دقيقة
        isIntervalSet = true;
      }
    } catch (error) {
      console.error("Failed to load the existing message:", error.message);
      notifyDevelopers(`Failed to load the existing message: ${error.message}`);
    }
  }
});

async function checkDevelopersOnline() {
  for (const [name, id] of Object.entries(config.developerIds)) {
    try {
      const user = await client.users.fetch(id);
      console.log(`Developer ${name} (${user.tag}) is added to the system.`);
    } catch (error) {
      console.error(
        `Failed to fetch developer ${name} (${id}):`,
        error.message
      );
    }
  }
...
client.on("messageCreate", async (message) => {
  ...
  if (command === "role-status") {
    try {
      // إرسال الرسالة الأولى أو تحديث الرسالة الموجودة
      if (!embedMessageId) {
        const embed = await generateEmbed(message.guild);
        const sentMessage = await message.channel.send({ embeds: [embed] });
        embedMessageId = sentMessage.id; // تخزين الـ ID
        config.channelId = message.channel.id; // تخزين الـ channel ID
      } else {
        await updateEmbed(); // تحديث الرسالة في حال كانت موجودة
      }

      // تحديث الرسالة كل دقيقة (فقط إذا لم يتم إنشاء الـ interval بالفعل)
      if (!isIntervalSet) {
        setInterval(updateEmbed, 60000); // التحديث كل دقيقة
        isIntervalSet = true; // تعيين المتغير إلى true لتجنب إنشاء interval جديد
      }
    } catch (error) {
      console.error("Error generating the embed:", error.message);
      message.reply("An error occurred while generating the role status.");
      notifyDevelopers(`Error generating the embed: ${error.message}`); // إعلام المطورين
    }
  }
});

// وظيفة لتحديث الرسالة
async function updateEmbed() {
  try {
    const channel = await client.channels.fetch(config.channelId);
    const message = await channel.messages.fetch(embedMessageId);
    const embed = await generateEmbed(channel.guild); // توليد الـ embed الجديد
    await message.edit({ embeds: [embed] }); // تحديث الرسالة
  } catch (error) {
    console.error("Error updating the embed:", error.message);
    notifyDevelopers(`Error updating the embed: ${error.message}`); // إعلام المطورين
  }
}

async function generateEmbed(guild) {
  // تحديث الكاش لجميع الأعضاء
  await guild.members.fetch();

  const embed = new MessageEmbed()
    .setImage(
      "https://cdn.discordapp.com/attachments/1254132794293092363/1326386282120872016/0211.webp?ex=677f3cfc&is=677deb7c&hm=a63a037d72d5dd72a0cd75bcabadf43a6690d99d7326660212b338e1c47cfbe2&"
    )
    .setAuthor({
      name: `🥇 احصائيات العائلات`,
  ...
  **💎 الاول خبرة**  
  #1 **Al.Gharbawi** Family [3.320 XP]

  **💡 الاول نقاط**  
  #1 **Al.Gharbawi** .. [140 Points]
  
  **🚀 الاعلى تفاعل بالديسكورد**  
  #1 **El.Bkhet** Family 
  \n`;
  ...
  for (const roleId of config.roles) {
    ...
    let offlineMembersCount = 0;

    // احصاء الأعضاء بناءً على حالة وجودهم في الخدمة
    membersInRole.forEach((member) => {
      const status = member.presence?.status;

      // هنا بنحسب الأعضاء اللي في الخدمة بجميع الحالات المعروفة
      if (
        status === "online" ||
        status === "idle" ||
        status === "dnd" ||
    ...
    // إضافة عدد الأعضاء بجانب اسم الرول مع الاموجي
    const emoji = config.roleEmojis[roleId] || "⚪"; // إذا لم يوجد إيموجي معرف، يتم استخدام الإيموجي الافتراضي
    description += `- (الأعضاء ${membersInRole.size}) **<@&${role.id}>**\n\n`;
  }

  embed.setDescription(description || "No roles found or no members assigned.");
  return embed;
}

// وظيفة لإعلام المطورين بالأخطاء
async function notifyDevelopers(errorMessage) {
  for (const [name, id] of Object.entries(config.developerIds)) {
    try {
      const developer = await client.users.fetch(id);
      await developer.send(`**Error Alert:**\n${errorMessage}`);
    } catch (error) {
      console.error(
        `Failed to notify developer ${name} (${id}):`,
        error.message
      );
    }
  }
...
async function sendTimeEmbed(channel) {
  ...
  const embed = new MessageEmbed()
    .setTitle(
      "> <a:ejgif1004:1241743499678973952> بيانات الوقت و التاريخ بتوقيت `القاهرة` <a:ejgif1005:1241743503403253860>"
    )
    .setFooter({
    ...
    .addFields(
      ...
      {
        name: "التاريخ",
        value: `\`\`\`${getCairoGregorianDate()} AD\`\`\``,
        inline: true,
      },
      {
        name: "ايام الاسبوع",
        value: `\`\`\`${getCairoDayOfWeek()}\`\`\``,
        inline: true,
      },
      {
        name: "الموسم الحالي",
      ...
      {
        name: "درجة حرارة بالقاهرة",
        value: `\`\`\`${getCurrentTemperature()}°C\`\`\``,
        inline: true,
      },
      {
        name: "الرطوبة بالقاهرة",
        value: `\`\`\`${getCurrentHumidity()}%\`\`\``,
        inline: true,
      },
      {
        name: "الساعات المتبقية لهذا اليوم",
        value: `\`\`\`${hoursRemainingToday}\`\`\``,
        inline: true,
      },
      {
        name: "أيام السنة المنقضية",
        value: `\`\`\`${daysElapsed}\`\`\``,
        inline: true,
      },
      {
        name: "أسابيع السنة المنقضية",
        value: `\`\`\`${weeksElapsed}\`\`\``,
        inline: true,
      },
      {
        name: "أشهر السنة المنقضية",
        value: `\`\`\`${monthsElapsed}\`\`\``,
        inline: true,
      },
      {
        name: "ساعات السنة المنقضية",
        value: `\`\`\`${hoursElapsedThisYear}\`\`\``,
        inline: true,
      }
    );
...
async function generateLeaderboardEmbed(
  ...
  const leaderboardEmbed = new MessageEmbed()
    ...
    .setTimestamp()
    .setAuthor({
      name: `قائمة التصنيف الخاصة بمستوي الاعضاء 📋`,
      iconURL: message.guild.iconURL({ dynamic: true }),
    })
    .setFooter(
      `${message.author.tag} • صفحة ${page} من ${Math.ceil(
...
async function generareCanvas5(
  member,
  levelUps,
  leaderboardPosition,
  oldLevel
  ...
  try {
    ...
    let canvas = new Canvas(852, 324)
      ...
      .printText(
        `${oldLevel} > ${levelUps}・#${leaderboardPosition}`,
        710,
        170
      );
...
client.on("messageCreate", async (message) => {
  ...
  if (command === `${prefix}add-xp`) {
    ...
    if (!userId || !xpAmount || isNaN(xpAmount)) {
      message.reply(
        "Invalid command usage. Correct usage: !add-xp [@mention or id] [xp amount]"
      );
      return;
    }
  ...
  if (command === `${prefix}add-level`) {
    ...
    if (!userId || !levelAmount || isNaN(levelAmount)) {
      message.reply(
        "Invalid command usage. Correct usage: !add-level [@mention or id] [level amount]"
      );
      return;
    }
  ...
  if (command === `${prefix}reset-level-member`) {
    ...
    if (!userId) {
      message.reply(
        "Invalid command usage. Correct usage: !reset-leveling [@mention or id]"
      );
      return;
    }
  ...
  if (command === `${prefix}xp`) {
    ...
    const embed = new MessageEmbed()
      ...
      .addFields(
        ...
        {
          name: "Joined Server",
          value: `\`\`\`${moment(user.joinedAt).format("YYYY/MM/DD")}\`\`\``,
          inline: true,
        }
      );
  ...
  if (command === `${prefix}top`) {
    ...
    const row = new MessageActionRow().addComponents(
      ...
      new MessageButton()
        ...
        .setEmoji("▶️")
        .setStyle("PRIMARY"),
      new MessageButton().setCustomId("last").setEmoji("⏩").setStyle("PRIMARY")
    );
    ...
    const filter = (i) =>
      ["first", "prev", "home", "next", "last"].includes(i.customId) &&
  ...
  if (levelUps > (userData[`${authorId}_level`] || 0)) {
    ...
    if (member) {
      try {
        let buffer_attach = await generareCanvas5(
          member,
          levelUps,
          leaderboardPosition,
          oldLevel
        ...
        levelUpChannel.send({
          content: `**<a:ejgif1015:1241777034531831808> ${levelUps} <a:ejgif1015:1241777034531831808>**ㅤ**لقد وصلت الآن إلى المستوى**ㅤ**<a:ejgif1034:1246101314224521248> ${member} <a:ejgif1034:1246101314224521248>**ㅤ**تهانينا**`,
          files: [attachment],
        });
      } catch (error) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////// Azkar System
const channelId = "1329215057938219070"; // حط هنا ID الروم
const guildId = "740299333697536061"; // حط هنا ID السيرفر
const images = [
  "",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087731476004978/azkar_21.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087722076442736/azkar_20.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087711431557140/azkar_19.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087700152946698/azkar_18.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087690061578280/azkar_17.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294472084983808/azkar_6.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087667785502801/azkar_15.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1331087678862659604/azkar_16.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294484726485013/azkar_14.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294498282475540/azkar_12.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294513172381706/azkar_7.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294531438444545/azkar_9.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294548719112352/azkar_10.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294596223795280/azkar_3.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294682642976819/azkar_1.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294742378381372/azkar_13.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294755120681052/azkar_11.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294767598735543/azkar_5.webp",
  "https://cdn.discordapp.com/attachments/1254132794293092363/1329294787739783268/azkar_8.webp",
];
...
client.once("ready", async () => {
  const guild = client.guilds.cache.get(guildId); // استرجاع السيرفر
  ...
  setInterval(async () => {
    ...
    const embed = new MessageEmbed()
      .setAuthor({
        name: `اللهُم اجعلنا من الذاكرين ❤️`,
        iconURL:
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.toLowerCase() === `${prefix}report-system`) {
    ...
    const embed = new MessageEmbed()
      .setTitle(
        "> <a:ejgif1036:1250132334502739979> Submit A Report <a:ejgif1006:1241743608617504788>"
      )
      .setThumbnail(message.guild.iconURL({ dynamic: true, size: 4096 }))
      .setImage(
        "https://cdn.discordapp.com/attachments/1232668066069086248/1237501284551229511/E77wRD1KOLAfsd4tp6_standard.gif?ex=663be061&is=663a8ee1&hm=91a04116ef47ac24d61a2a8dea69fe3f2fa3c56d770a5122efe27ba470b3075a&"
      )
      .setDescription(
...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isButton()) {
    if (interaction.customId === "accept_sug") {
      ...
      embed.fields.find((field) => field.name === "Status").value =
        "✅ Accepted";
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === "report-server-modal") {
      ...
      const response2 = interaction.fields.getTextInputValue(
        "2report-server-input"
      );
      const response3 = interaction.fields.getTextInputValue(
        "3report-server-input"
      );
      const startTimestamp = Math.floor(Date.now() / 1000) - 27;
      let currentTime = new Date();
      currentTime.setHours(currentTime.getHours() + 1);
      const userId = interaction.user.id;
      const egyptianDate = new Date().toLocaleDateString("en-US", {
        timeZone: "Africa/Cairo",
      });
      const egyptianDate2 = currentTime.toLocaleTimeString("en-EG", {
        timeZone: "Africa/Cairo",
        hour12: true,
        hour: "numeric",
        minute: "numeric",
      ...
      const embed2 = new MessageEmbed()
        ...
        .addFields(
          ...
          {
            name: "Report Data",
            value: `\`\`\`${egyptianDate2},${egyptianDate}\`\`\``,
            inline: true,
          }
        );
      ...
      const embed = new MessageEmbed()
        .setColor("#2c2c34")
        .setTitle(
          "> Your notification has been successfully sent to the administrators \n> Your report is being reviewed"
        );
...
client.on("messageCreate", async (message) => {
  ...
  if (botMentioned) {
    ...
    const embed = new MessageEmbed()
      .setColor("#2c2c34")
      .setTitle(
        `My commands : \`${prefix}help\` or </help:1239056012434997341>`
      );
...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isButton()) {
    if (interaction.customId === "accept_sug22") {
      ...
      const requiredRoles = ["1218283751293849631"];
      ...
      const suggestionChannel =
        interaction.guild.channels.cache.get(sugaccptorreject);
      if (suggestionChannel) {
        ...
        const acceptEmbed = new Discord.MessageEmbed()
          ...
          .addFields(
            ...
            {
              name: "تم قبول الاقتراح بواسطة",
              value: `┕${suggestedBy}`,
              inline: true,
            },
            {
              name: "تاريخ قبول الاقتراح",
              value: `┕\`${egyptianDate2},${egyptianDate}\``,
              inline: true,
            },
            { name: `\u2003`, value: `\u2003`, inline: false },
            {
              name: "الوقت المنقضي منذ",
              value: `┕<t:${startTimestamp}:R>`,
              inline: true,
            },
            {
              name: "الاقتراح المقبول",
              value: `[رابط الاقتراح](${sourceMessage.url})┕`,
              inline: true,
            }
          );
    ...
    if (interaction.customId === "unaccept_sug22") {
      ...
      if (!isAdmin && !hasRequiredRole) {
        return interaction.reply({
          content: "Sorry, you don't have permission to perform this action.",
          ephemeral: true,
        });
      }
      ...
      if (suggestionChannel) {
        ...
        const unacceptEmbed = new Discord.MessageEmbed()
          ...
          .addFields(
            { name: `\u2003`, value: `\u2003`, inline: false },
            {
              name: "تم رفض الاقتراح بواسطة",
              value: `┕${suggestedBy}`,
              inline: true,
            },
            {
              name: "تاريخ رفض الاقتراح",
              value: `┕\`${egyptianDate2},${egyptianDate}\``,
              inline: true,
            },
            { name: `\u2003`, value: `\u2003`, inline: false },
            {
              name: "الوقت المنقضي منذ العملية",
              value: `┕<t:${startTimestamp}:R>`,
              inline: true,
            },
            {
              name: "الاقتراح المرفوض",
              value: `[رابط الاقتراح](${sourceMessage.url})┕`,
              inline: true,
            }
          );
    ...
    // Check if the button is part of the voting system
    if (
      interaction.customId === "like_sug" ||
      interaction.customId === "dis_sug"
    ) {
    ...
    if (interaction.customId === "like_sug") {
      ...
      const logEmbed = new MessageEmbed()
        ...
        .setAuthor({
          name: `تم التفاعل مع هذا الاقتراح`,
        ...
        .addFields(
          { name: `\u2003`, value: `\u2003`, inline: false },
          {
            name: "تم التفاعل بواسطة",
            value: `<@${votedMember}>`,
            inline: true,
          },
          {
            name: "تاريخ التفاعل",
            value: `<t:${Math.floor(startTimestamp / 1000)}:R>`,
            inline: true,
          },
          { name: `\u2003`, value: `\u2003`, inline: false },
          {
            name: `\u2003`,
            value: `\`\`\`diff\n+👍 ${currentLikes + 1} تفاعلات حالية\`\`\``,
            inline: false,
          },
          { name: `\u2003`, value: `\u2003`, inline: false },
          {
    ...
    if (interaction.customId === "dis_sug") {
      ...
      const logEmbed = new MessageEmbed()
        ...
        .addFields(
          ...
          {
            name: `\u2003`,
            value: `\`\`\`diff\n-👎 ${currentDislikes + 1} تفاعلات حالية\`\`\``,
            inline: false,
          },
          { name: `\u2003`, value: `\u2003`, inline: false },
          {
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === "suggestions-modal") {
      const response =
        interaction.fields.getTextInputValue("suggestions-input");
      const response2 =
        interaction.fields.getTextInputValue("2suggestions-input");
      const startTimestamp = Math.floor(Date.now() / 1000) - 32;
      let currentTime = new Date();
      currentTime.setHours(currentTime.getHours() + 1);
      const userId = interaction.user.id;
      ...
      const embed2 = new MessageEmbed()
        ...
        .setDescription(
          `**عنوان الاقتراح** \`\`\`${response}\`\`\` \n**فكرة الاقتراح** \`\`\`${response2}\`\`\``
        )
        .addFields(
          { name: "الحالة", value: `⏳ قيد المراجعة`, inline: true },
          {
            name: "تاريخ تقديم الاقتراح",
            value: `┕\`${egyptianDate2},${egyptianDate}\``,
            inline: true,
          },
          {
            name: "المقترح تقديمه من قبل",
            value: `<@${userId}>`,
            inline: true,
          },
          {
      ...
      const accept_sug = new MessageButton()
        ...
        .setLabel("قبول")
        .setEmoji("<:859388130411282442:1219455979972984842>")
        .setStyle("SUCCESS");

      const unaccept_sug = new MessageButton()
        .setCustomId("unaccept_sug22")
        .setLabel("رفض")
        .setEmoji("<:859388130636988436:1219455995667939391>")
        .setStyle("DANGER");
      ...
      const report_sug = new MessageButton()
        ...
        .setEmoji("<:emoji_29:1224189321595195463>")
        .setStyle("DANGER");
      ...
      interaction
        ...
        .then(() => {
          ...
          const channel =
            interaction.client.channels.cache.get(suggestionschannel);
          if (channel && channel.isText()) {
            channel.send({ embeds: [embed2], components: [row1] });
          } else {
    ...
    if (interaction.isButton()) {
      if (interaction.customId === "rename-ticket-button") {
        ...
        if (!hasClaimPermission(interaction.member)) {
          await interaction.reply({
            content: "ليس لديك الصلاحية لأداء هذا الإجراء",
            ephemeral: true,
          });
          return;
        }
      ...
      if (interaction.customId === "rename-ticket-modal") {
        ...
        if (interaction.member.permissions.has("MANAGE_CHANNELS")) {
          ...
          const embed = new MessageEmbed()
            ...
            .addFields(
              {
                name: "الاسم القديم للتذكرة",
                value: `\`\`\`diff\n- [ ${oldTicketName} ]\`\`\``,
                inline: true,
              },
              {
                name: "الاسم الجديد للتذكرة",
                value: `\`\`\`diff\n+ [ ${response} ]\`\`\``,
                inline: true,
              }
            );
          ...
          await interaction.reply({
            content: "لا أملك الصلاحية لإعادة تسمية القنوات!",
            ephemeral: true,
          });
        }
      }
...
let ticketCounter = 1;
const bannedRoleId = "1327815619394928722"; // الرول المحظورة
...
claimPermissions.add("1317975784245432461");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "close_id_note") {
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        "- `إغلاق التذكرة` سيقوم بإغلاق هذه التذكرة عنك\n- `إلغاء` سيقوم بإلغاء عملية إغلاق هذه التذكرة"
      );
    ...
    collector.on("end", async () => {
      if (message && !message.deleted) {
        await message.delete().catch(console.error); // Make sure to delete the message in case the button press event doesn't occur
      } else {
        console.log("Message is either undefined or already deleted.");
      }
    });
    ...
    try {
      ...
      const loadingMessage = await interaction.followUp({
        embeds: [
          new MessageEmbed()
            .setDescription("**جاري التجميل - يرجى الانتظار... 🛡**")
            .setColor("#2c2c34"),
        ],
      });
      ...
      setTimeout(async () => {
        ...
        const permissionOverwrites = channel.permissionOverwrites.cache.filter(
        ...
        // إضافة ساعة واحدة
        currentTime.setHours(currentTime.getHours() + 1);
        const saveticketdate = new Date().toLocaleDateString("en-US", {
          timeZone: "Africa/Cairo",
        });
        const saveticketdate2 = currentTime.toLocaleTimeString("en-EG", {
          timeZone: "Africa/Cairo",
          hour12: true,
          hour: "numeric",
          minute: "numeric",
        ...
        const mentionListFormatted = mentionList.map(
          (mention, index) => `${index + 1}. ${mention}`
        );
        ...
        const closedEmbed = new MessageEmbed()
          ...
          .setDescription(
            `**لقد تم أخفاء التذكرة كلاً من**\n${mentionListString}`
          )
          .addFields(
            {
              name: "تم إغلاق هذه التذكرة بواسطة",
              value: `${interaction.member}`,
              inline: true,
            },
            { name: "\u2003", value: `\u2003`, inline: true },
            {
              name: "تاريخ أغلاق التذكرة",
              value: `**\`${saveticketdate2},${saveticketdate}\`**`,
              inline: true,
            }
          );
        ...
        const embed = interaction.message.embeds[0];
        const ticketOwnerField = embed.fields.find(
          (field) => field.name === "منشئ التذكرة"
        );
        const ticketOwnerValue = ticketOwnerField
          ? ticketOwnerField.value
          : "غير معروف";
        const ticketOwnerField2 = embed.fields.find(
          (field) => field.name === "قسم التذكرة"
        );
        const ticketOwnerValue2 = ticketOwnerField2
          ? ticketOwnerField2.value
          : "غير معروف";
        const ticketOwnerField3 = embed.fields.find(
          (field) => field.name === "الدعم المطلوب"
        );
        const ticketOwnerValue3 = ticketOwnerField3
          ? ticketOwnerField3.value
          : "غير معروف";

        // Get the user who clicked the button
        const claimTicket = interaction.user;
        const startTimestamp = Math.floor(Date.now() / 1000) - 35;
        const saveticketdate22 = new Date().toLocaleDateString("en-US", {
          timeZone: "Africa/Cairo",
        });
        const saveticketdate23 = currentTime2.toLocaleTimeString("en-EG", {
          timeZone: "Africa/Cairo",
          hour12: true,
          hour: "numeric",
          minute: "numeric",
        ...
        const embed9 = new MessageEmbed()
          ...
          .addFields(
            {
              name: "منشئ هذه التذكرة",
              value: `\`مذال قيد التطوير\``,
              inline: true,
            },
            {
              name: "تم إغلاق التذكرة بواسطة",
              value: `${mention}`,
              inline: true,
            },
            {
              name: "تم إستلام التذكرة بواسطة",
              value: `\`مذال قيد التطوير\``,
              inline: true,
            },
            ...
            {
              name: "تاريخ التذكرة",
              value: `**\`${saveticketdate23},${saveticketdate22}\`**`,
              inline: true,
            }
          )
        ...
        if (!destinationChannel) {
          ...
          return await interaction.followUp({
            content: "القناة المخصصة غير موجودة",
            ephemeral: true,
          });
        }
  ...
  // التحقق من وجود الرول المحظورة
  if (member.roles.cache.has(bannedRoleId)) {
    return interaction.reply({
      content:
        "🚫 **عذرًا!** لا يمكنك فتح تذكرة لأنك محظور من استخدام نظام التذاكر.",
      ephemeral: true,
    });
  }
  ...
  if (interaction.customId === "ticket_panel") {
    ...
    const selectedDepartment = selectMenuOptions.find(
    ...
    try {
      ...
      fs.readFile(ticketsFilePath, "utf8", async (err, data) => {
        ...
        const channel = await guild.channels.create(
          `${selectedDepartment.label}-${ticketCounter}`,
          {
            type: "text",
        ...
        const embedopen1 = new MessageEmbed()
          .setAuthor({
            name: `${member.guild.name} مرحبًا بك في خادم`,
            iconURL: interaction.guild.iconURL({ dynamic: true }),
          })
          /*//ticket image
          ...
          .addFields(
            ...
            {
              name: "الدعم المطلوب",
              value: `${selectedDepartment.rolesupport}`,
              inline: true,
            },
            ...
            {
              name: "التذكرة منذ",
              value: `┕<t:${Math.floor(Date.now() / 1000) - 85}:R>`,
              inline: true,
            },
            {
              name: "تاريخ الانضمام لديسكورد",
              value: `┕<t:${Math.floor(
        ...
        channel.send({
          content: `||${member} - ${selectedDepartment.rolesupport}||`,
          embeds: [embedopen1],
...
tracker.on("guildMemberAdd", async (member, inviter, invite, error) => {
  ...
  let embed = new MessageEmbed()
    .setTitle(
      `> <:ejpic1008:1241773699544252447> Welcome To __${member.guild.name}__ Server <:ejpic1001:1241773680556507267>`
    )
    .addFields(
      {
        name: "<:ejpic1011:1241773707542659224> Welcome",
        value: `${member.user}`,
        inline: true,
      },
      {
        name: "<:ejpic1014:1241773715801509909> Invited By",
        value: `<@!${inviter.id}>`,
        inline: true,
      },
      {
        name: "<:ejpic1005:1241773691591987410> Rules",
        value: `<#1150611319775580171>`,
        inline: true,
      }, // Using the fetched description here
      {
        name: "<:ejpic1010:1241773705189658746> User ID",
        value: `\`\`${member.user.id}\`\``,
        inline: true,
      },
      {
        name: "<:ejpic1003:1241773685891666051> Member Count",
        value: `\`\`${memberCount}\`\``,
        inline: true,
      },
      {
        name: "<:ejpic1002:1241773683249254523> Invite Number",
        value: `\`\`${invite.count}\`\``,
        inline: true,
      },
      {
        name: "<:ejpic1009:1241773702471880785> Joined Server",
        value: `<t:${startTimestamp}:R>`,
        inline: true,
      },
      {
        name: "<:ejpic1015:1241773719123267645> Joined Discord",
        value: `<t:${Math.floor(member.user.createdAt / 1000)}:R>`,
        inline: true,
      },
      {
        name: "<:ejpic1006:1241773694477668473> Member User",
        value: `\`\`${member.user.username}\`\``,
        inline: true,
      },
      {
        name: "<:ejpic1007:1241773697036193902> Server Support",
        value: `[Click Here](https://dsc.gg/clipper-tv)`,
        inline: true,
      },
      {
        name: "<:ejpic1012:1241773710910816256> Instagram",
        value: `[Click Here](https://www.instagram.com/ahm.depression/reels)`,
        inline: true,
      },
      {
        name: "<:ejpic1012:1241773710910816256> Twitter",
        value: `[Click Here](https://twitter.com/ahm_depression)`,
        inline: true,
      }
    )
    .setColor("#2F3136")
  ...
  if (welcomeChannel) {
    welcomeChannel.send({
      ...
      embeds: [embed],
      files: [attachment],
    }); // إرسال الـ embed في الروم المحدد
  } else {
...
client.on("messageCreate", async (message) => {
  if (message.content === `${prefix}profile`) {
    ...
    const creatingCardMessage = await message.reply(
      "> <a:ejgif1004:1241743499678973952> **Creating Your Profile Card...**"
    );
    await message.channel.sendTyping();
    ...
    message.reply({
      content: `> <a:ejgif1033:1242349759298015334> **ملاحظة : هذا الأمر لم يكتمل بعد، ومازال قيد التطوير** <a:ejgif1032:1242349755728789504>`,
      files: [attachment],
    }); // إرسال الصورة المرفقة فقط في الروم المحدد
  }
});
async function generareCanvas2(member) {
  ...
  const memberSinceServerEnglish = member.joinedAt.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  const memberSinceDiscordEnglish = member.user.createdAt.toLocaleDateString(
    "en-US",
  ...
  if (message.content === `${prefix}rank`) {
    ...
    if (cooldowns.has(authorId)) {
      ...
      const warningMessage = await message.reply(
        `ثواني قبل استخدام هذا الأمر مرة أخرى ${remainingTime.toFixed(
          1
        )} انتظر من فضلك.`
      );
    ...
    // إرسال رسالة قبل بدء إنشاء الصورة
    const creatingCardMessage = await message.reply(
      "> <a:ejgif1004:1241743499678973952> **...جاري إنشاء بطاقة المستوي الشخصي**"
    );

    // إنشاء الصورة
    let buffer_attach = await generateCanvas3(
      message.member,
      level,
      xp,
      leaderboardPosition
